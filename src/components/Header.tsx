  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-white border-b border-gray-200">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-4">
        <button onClick={() => {
          // Якщо вже на головній без query/hash — просто вгору. Інакше повний
          // перехід на головну, що скидає поточну сторінку та стан (як у футері).
        ...
        <div className="hidden md:flex items-center gap-3">
          <div className="relative">
            <button onClick={() => { setLangOpen((o) => !o); setCurrOpen(false); }} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors px-2 py-1.5">
            ...
            {langOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
                  {LANGS.map((l) => (
                    <button key={l.code} onClick={() => { onLangChange(l.code); close(); }} className={cn("w-full text-left px-4 py-2 text-sm flex items-center gap-2.5 hover:bg-gray-50 transition-colors", lang === l.code ? "text-emerald-700 font-semibold" : "text-gray-700")}>
                      {l.flag}{l.label}
                    </button>
                  ))}
                </div>
          ...
          <div className="relative">
            <button onClick={() => { setCurrOpen((o) => !o); setLangOpen(false); }} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors px-2 py-1.5">
            ...
            {currOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
                  {CURRENCY_LIST.map((c) => (
                    <button key={c.code} onClick={() => { onCurrencyChange(c.code); close(); }} className={cn("w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors", currency === c.code ? "text-emerald-700 font-semibold" : "text-gray-700")}>
                      {c.sym} {c.code}
                    </button>
                  ))}
                </div>
          ...
          {user ? (
            <div className="flex items-center gap-3 pl-3 border-l border-gray-200">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-900 max-w-[90px] truncate">{user.name.split(" ")[0]}</span>
                <button onClick={() => logout()} className="text-gray-400 hover:text-rose-500 transition-colors ml-0.5" title={t.logout}>
            ...
            <>
              <div className="flex items-center gap-2.5 p-3 bg-gray-50 rounded-xl mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{user.role === "PROVIDER" ? "Фахівець" : user.role === "ADMIN" ? "Адмін" : "Клієнт"}</p>
                </div>
              </div>
